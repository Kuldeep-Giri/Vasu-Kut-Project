...
export class SellerRegistrationComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // Auto update code based on country
    this.registerForm.get('Country')?.valueChanges.subscribe(selectedCountry => {
      const match = this.countries.find(c => c.name === selectedCountry);
      if (match) {
        this.registerForm.patchValue({ code: match.code });
      }
    });
  }
  ...
  toggleUserType(type: string) {
    ...
    this.registerForm.get('role')?.setValue(type);
  }
      ...
      this.authService.register(this.registerForm.value).subscribe({
        ...
        error: (error) => {
          if (error.error && error.error.message) {
            this.toastr.error(error.error.message); // Shows "Email already exists"
          } else if (Array.isArray(error.error)) {
            const firstError = error.error[0]?.description || 'Registration failed';
